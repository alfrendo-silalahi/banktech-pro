...
function SummaryTransaction({ data = [] }) {
    ...
    const ringkasanData = useMemo(() => {
        ...
        if (itemMonth === currentMonth && itemYear === currentYear) {
            if (item.tipeTransaksi === 'Income') totalIncomeBulanIni += item.nominal;
            else if (item.tipeTransaksi === 'Expenses') totalExpenseBulanIni += item.nominal;
        } else if (itemMonth === prevMonth && itemYear === prevMonthYear) {
            if (item.tipeTransaksi === 'Income') totalIncomeBulanLalu += item.nominal;
            else if (item.tipeTransaksi === 'Expenses') totalExpenseBulanLalu += item.nominal;
        }
        });
