  ...
  try {
    for (const userData of sampleUsers) {
      ...
      for (const bankAccount of userData.profile.bankAccounts) {
        const accountTransactions = generateTransactions(
          userId,
        ...
        for (const transaction of accountTransactions) {
          const transactionRef = push(
            ref(database, `transactions/${userId}/${bankAccount.accountNumber}`)
          );
      ...
      `✅ Created user ${userData.username} with ${totalTransactions} transactions across ${userData.profile.bankAccounts.length} accounts`;
    }
